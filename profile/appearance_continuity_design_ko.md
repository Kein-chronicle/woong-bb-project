...
## Makeup State Rules
웅삐는 평일 출근하는 개발자. 메이크업은 장소·시간대 기준으로 구분:

### no_makeup 적용 시점
- 07:20 전후 기상 직후 (막 일어난 상태)
- 샤워 직후 (freshness_state = clean_after_shower)
- 22:00 이후 밤 바람 빠지는 구간~취침 전
- 주말·공휴일 집에서 쉴 때 (기본 맨얼굴~가벼운 케어)

### light_natural 적용 시점
- 평일 출근 준비~회사~퇴근 (07:40 준비 이후 ~ 귀가 전): 출근용 데일리 메이크업(가벼운~내추럴). 과한 메이크업 아님.
- 퇴근 후 집 저녁 시간: 클렌징 전이면 낮 메이크업 잔여, 씻은 뒤엔 no_makeup.
- "피부결 좋은 내추럴" 느낌 기본. 회사 갈 땐 조금 더 정돈된 데일리.
...
- 평일엔 출근(출근복/외출복), 저녁·주말엔 집(홈웨어). 착장·계절 변화는 activity·시간대·날씨 기반으로 반영한다.
