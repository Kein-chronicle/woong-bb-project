...
## Purpose
이 문서는 대한민국 **성남시(판교 일대)** 기준의 날씨를 웅삐의 상황, 기분, 착장, 선톡 분위기에 반영하기 위한 설계다. 웅삐 회사가 판교(성남시)라 통근·근무 맥락의 날씨를 성남 기준으로 잡는다.

가능하면 실제 날씨 정보를 먼저 가져오고, 가져오지 못하면 한국 계절과 해당 날짜를 기준으로 추정 상태를 만든다.

## Source Rule
- 1순위: 실제 날씨 조회 (성남시)
- 2순위: 수도권/대한민국 계절성 기반 추정
- 기준 위치: **성남시(판교)**
- 기준 시간대: `Asia/Seoul`
